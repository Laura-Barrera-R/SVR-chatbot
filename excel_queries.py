...
def closed_events_oct_nov(df, show_chart=True):
    df = df.copy()
    fecha_col = df['event_close_datetime']
    if pd.api.types.is_numeric_dtype(fecha_col):
        df['event_close_datetime'] = pd.to_datetime(fecha_col, unit='D', origin='1899-12-30')
    else:
        df['event_close_datetime'] = fecha_col.astype(str).str.strip()
        df['event_close_datetime'] = pd.to_datetime(df['event_close_datetime'], errors='coerce')
        if df['event_close_datetime'].isna().all():
            df['event_close_datetime'] = pd.to_datetime(
                df['event_close_datetime'], format='%m/%d/%Y %H:%M', errors='coerce'
            )
        if df['event_close_datetime'].isna().all():
            df['event_close_datetime'] = pd.to_datetime(
                df['event_close_datetime'], format='%m/%d/%Y %H:%M:%S', errors='coerce'
            )
    
    # Mask para filtrar closed en oct y nov
    mask = (
        df['event_state'].str.upper() == "CLOSED"
    ) & (
        df['event_close_datetime'].notnull() &
        df['event_close_datetime'].dt.month.isin([10, 11])
    )
    filtered = df[mask]
    counts = filtered['event_close_datetime'].dt.month.value_counts().sort_index()
...
def open_tickets_by_month(df, show_chart=True):
    ...
    if pd.api.types.is_numeric_dtype(fecha_col):
        ...
        if df['event_start_datetime'].isna().all():
            df['event_start_datetime'] = pd.to_datetime(
                df['event_start_datetime'], format='%m/%d/%Y %H:%M', errors='coerce'
            )
        if df['event_start_datetime'].isna().all():
            df['event_start_datetime'] = pd.to_datetime(
                df['event_start_datetime'], format='%m/%d/%Y %H:%M:%S', errors='coerce'
            )
